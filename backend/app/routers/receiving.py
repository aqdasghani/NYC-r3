...
from ..deps import get_db, get_stock_receiver
...
from ..models.database import InventoryBatch, InventoryTransaction, Product, User
...
async def scan_invoice(file: UploadFile = File(...), user: User = Depends(get_stock_receiver), db=Depends(get_db)):
    if not user.store_id: raise HTTPException(403, "User is not assigned to a store")
...
async def confirm_receipt(payload: ConfirmReceiptRequest, user: User = Depends(get_stock_receiver), db=Depends(get_db)):
    if not user.store_id: raise HTTPException(403, "User is not assigned to a store")
    created = []
    for item in payload.items:
        ...
        db.add(InventoryTransaction(
            store_id=user.store_id, product_id=item.product_id, batch_id=batch.id,
            tx_type="RECEIVE", quantity=item.quantity,
            note=f"Received via {batch.batch_number or 'GRN'}", performed_by=user.id,
        ))
    db.commit()
